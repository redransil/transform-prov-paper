...
\hline
\makecell[l]{\textbf{series} \\ \textit{(array)}} & \makecell[l]{An array of the CIDs of other \\ Types if this a series type. May \\ be null. Must not conflict with \\ Type.cid.} \\
\hline
...
\captionof{figure}[Types and assets]{Block diagram showing types and assets in Transform.Storage. (a) this type is defined by a schema which is referenced in its Type.cid field. (b) the series type $A \otimes B$ is defined by two other types referenced in its Type.series field. }
\label{typesAssets}
\end{center}
\begin{multicols}{2}
...
\eIf{T.series is not null} 
{
	\If {Type of D is not array}
	{
		\Return False
	}
	\For{index i of T.series}{
    		Run isTerm recursively on T.series[i], D.series[i]\\
		\If {isTerm returns false}
		{\Return False}
  }
    \tcc{We've checked that each element of D.series matches T.series}
    \Return True
    }{
\tcc{If this is a primitive type}
